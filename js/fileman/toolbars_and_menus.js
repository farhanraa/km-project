FR.initToolbar = function() {
	...
	FR.UI.actions = {
		...
		searchOpts: new Ext.Action({
			...
			menu: searchOptsMenu, disabled: true,
			handler: function() {
		...
		download: new Ext.Action({
			iconCls: 'fa fa-download fa-fw', hidden: true,
	...
	a.refresh = new FR.ContextAction({
		text: FR.T('Refresh'), iconCls: 'fa-refresh', overflowText: FR.T('Refresh'), action: 'refresh', requires: ['empty', function() {return FR.currentPath.length;}]
	});
	...
	a.locate = new FR.ContextAction({
		text: FR.T('Locate'), iconCls: 'fa-crosshairs', action: 'locate', requires: ['single', 'in-grid', 'not-virtual', {sections: ['recent', 'media', 'starred', 'shares', 'webLinked', 'search']}]
	});
	...
	a.label = new FR.ContextAction({
		text: FR.T('Label'), iconCls: 'fa-tag',	menu: {items: labels}, requires: ['non-empty', 'download', 'not-homefolder',
		function() {
	...
	a.addTags = new FR.ContextAction({
		text: FR.T('Add tags'), action: 'addTags', requires: [function(cm) {
			return User.perms.metadata && cm.target.length > 1;
		}]
	});
	a.addTags2 = new FR.ContextAction({
		text: FR.T('Tag files'), action: 'addTags', requires: [function(cm) {
			return User.perms.metadata && cm.target.length == 0;
		}]
	});
	a.addStar = new FR.ContextAction({
		...
		requires: [
			...
			function() {return (FR.currentSection != 'starred');},
			function(cm) {
				if (cm.location == 'grid') {
					var countStarred = 0;
	...
	a.props = new FR.ContextAction({
		text: FR.T('Properties'), iconCls: 'fa-cog', action: 'props', requires: [
			...
			'single', 'folder',
			function(cm) {
				if (['recent', 'myfiles', 'userWithShares', 'sharedFolder', 'starred', 'shares', 'webLinked'].indexOf(cm.section) != -1) {
	...
	a.copy = new FR.ContextAction({
		text: FR.T('Copy'), action: 'copy', requires: ['non-empty', 'section-not-trash', 'not-homefolder', 'download', function() {
			return (!User.perms.read_only && User.perms.upload);
		}]
	});
	...
	a.alog = new FR.ContextAction({
		text: FR.T('Activity log'), iconCls: 'fa-archive', action: 'activityLog', requires: [
			...
			function(cm) {
				...
				var s = cm.section;
				if (['myfiles', 'recent', 'starred', 'webLinked', 'shares'].indexOf(s) != -1) {
					return true;
	...
	a.extract = new FR.ContextAction({
		text: FR.T('Extract archive'), iconCls: 'fa-file-zip-o', action: 'extract', requires: ['file', 'single', 'download', 'create', function(cm){return (cm.target[0].filetype == 'arch');}],
		isMoreOptItem: true
	});
	...
	a.lock = new FR.ContextAction({
		text: FR.T('Lock'),	iconCls: 'fa-lock', action: 'lock', requires: ['alter', 'download', 'file', function() {return !Settings.free_mode;}]
	});
	a.unlock = new FR.ContextAction({
		text: FR.T('Unlock'), iconCls: 'fa-unlock-alt', action: 'unlock', requires: ['alter', 'download', 'file', function() {return !Settings.free_mode;}]
	});
					...
					if (prevReqCheck) {
						...
						if (User.perms.download) {
							if (this.location == 'tree') {
								...
								if (['recent', 'myfiles', 'starred', 'shares', 'recent', 'webLinked', 'search', 'userWithShares'].indexOf(FR.currentSection) != -1) {
									rs = true;
