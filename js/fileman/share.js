var FR = {
			...
			if (data.users) {
				Ext.each(data.users, function(user) {
					if (!treeRoot.findChild('uid', user.uid)) {
						FR.addToList({
							text: user.name,
							cls:'user', icon:'a/?uid='+user.uid, uid: user.uid
						});
					}
				});
			}
			if (data.groups) {
				Ext.each(data.groups, function(group) {
					if (!treeRoot.findChild('gid', group.gid)) {
						FR.addToList({
							text: group.name, cls:'group',
							iconCls:'group', gid: group.gid
						});
					}
				});
			}
	...
	addToList: function(data) {
		...
		if (!data.alias || data.alias == '') {data.alias = window.parent.FR.sharing.name;}
	...
	saveInfo: function() {
		...
		FR.UI.tree.rootNode.eachChild(function(node) {
			if (!node.disabled) {
				...
				if (a.alias && a.alias != window.parent.FR.sharing.name) {
					data.alias = a.alias;
				}
				if (a.gid) {
					data.type = 'group';
		...
		items: [
			{
				...
				items: [
					...
					},
					{
						...
						region: 'center',
						tbar: [
							{xtype: 'tbtext', style: 'font-weight:bold;font-size:14px;padding:6px 5px', id: 'optsTitle', text: '&nbsp;'},
							'->',
							{
								...
								items: [
									{
										...
										items: {
											...
											bodyStyle: 'padding:10px;font-size:12px;', labelWidth: 0,
											items: [
												{
