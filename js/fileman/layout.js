FR.initLayout = function() {
	...
	if (!User.perms.upload) {Ext.apply(FR.UI.NewBtnArea, {height: 23});}
	...
	FR.UI.explorerPanel = new Ext.Panel({
		...
		items: [
			...
			{
				layout: 'border', region: 'center', id: 'FR-Center-Region',
				items: [
					FR.UI.AudioPlayer,
					FR.UI.gridPanel
				]
	...
	FR.UI.window = new Ext.Viewport({
		...
		items:  {
			...
			items: {
				layout: 'border', region: 'center', bodyStyle: 'background-color: #F7F7F7;',
				items: [
					FR.UI.explorerPanel,
					FR.UI.infoPanel
				]
	...
	if (FR.UI.actions.logo.getWidth()+690 >= bodyWidth) {
		FR.UI.actions.logo.hide();
	}
