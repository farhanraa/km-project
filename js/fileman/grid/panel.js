FR.components.gridPanel = Ext.extend(Ext.grid.GridPanel, {
	...
	showTopMenu: function() {
		...
		Ext.iterate(['download', 'weblink', 'shareWithUsers', 'preview', 'remove'], function (k) {
		...
		if (FR.currentSection == 'myfiles') {
			if (this.countSel > 0) {
				...
				if (this.countSel == 1) {
					if (p.download) {
						if (!FR.currentSelectedFile.data.isFolder) {a.preview.show();}
						if (p.weblink) {a.weblink.show();}
			...
			if (this.countSel > 0) {
				if (p.download && f && f.download) {
					...
					if (this.countSel == 1) {
						if (!FR.currentSelectedFile.data.isFolder) {a.preview.show();}
						if (f.share && p.weblink) {a.weblink.show();}
