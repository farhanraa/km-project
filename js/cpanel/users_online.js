...
FR.usersOnline.grid = new Ext.grid.GridPanel({
	title: FR.T('Users currently online'), border: false,
	store: FR.usersOnline.store, hideHeaders: true, viewConfig: {emptyText: FR.T('There are no other users currently online')},
	cm: new Ext.grid.ColumnModel({
		defaults: {sortable: true},
		columns: [
			{header: FR.T("Id"), width: 30,  dataIndex: 'id', hidden: true},
			{header: FR.T("&nbsp;"), width: 30, dataIndex: 'id',
				renderer: function(id) {
					return '<img src="a/?uid='+id+'" width="16" height="16" valign="bottom" />';
				}
			},
