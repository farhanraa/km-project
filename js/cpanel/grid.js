...
FR.initGrid = function() {
	this.grid = {
		init: function() {
			this.actions = {
				add: new Ext.Action({
					text: FR.T('Add new'), hidden: true, cls: 'fr-btn-default', style: 'margin-left:5px',
				...
				view: new Ext.Action({
					text: FR.T('View'), cls: 'fr-btn-default', style: 'margin-left:5px',
				...
				openWebLink: new Ext.Action({
					text: FR.T('Open Web Link'), cls: 'fr-btn-default', style: 'margin-left:5px',
				...
				del: new Ext.Action({
					...
					iconCls: 'fa fa-fw fa-remove colorRed', hidden: true,
					handler: function() {
				...
				search: new Ext.Action({
					text: FR.T('Search'), style: 'margin-left:5px',
			...
			this.gridQuickSearchField = new Ext.form.TriggerField({
				...
				triggerClass: 'x-form-clear-trigger', hideTrigger: true, style: 'height:24px;',
				listeners: {
			...
			this.panel = new FR.components.Grid({
				id: 'gridTab',
				tbar: {
					items: [this.actions.add, this.actions.edit, this.actions.view, this.actions.openWebLink, this.actions.search, this.actions.loginAs, this.actions.viewLog, '->', this.actions.deActivate, this.actions.del, this.actions.archiveLogs, this.spacer, this.gridQuickSearchField],
					enableOverflow: true
				}
			});
		},
