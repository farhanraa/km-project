...
var FR = {
	...
	utils: {
		getAjaxOutput: function (url, params, loadingEl) {
			if (this.checkingPath) {return false;}
		...
		users: {
			...
			deleteAction: function(sel) {
				FR.deleteUsersDlg = new Ext.Window({
					...
					minimizable: false, maximizable: false, modal: true, buttonAlign: 'right', width:450,
					footer: true, closable: false, layout: 'fit', bodyBorder: false, bodyStyle: 'padding:0 0 15px 24px',
					...
					buttons: [
						...
						{text: FR.T('Cancel'), cls: 'fr-btn-default', style: 'margin-left:15px', handler: function(){FR.deleteUsersDlg.close();}, scope: this}
					]
				}).show();
			}
		},
