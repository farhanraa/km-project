var FR = {
	...
	initForm: function() {
		...
		this.win = new Ext.Window({
			...
			listeners: {'show': function(){
				...
				if (Settings.ui_login_logo.length) {
					Ext.fly('loginLogoImage').show().setStyle('background-image', 'url("'+Settings.ui_login_logo+'")')
						.setHeight(this.body.getHeight());

					/* adjust small logos */
					Ext.get(document.createElement('img')).on('load', function () {if (this.dom) {
						if (this.dom.width < 169 && this.dom.height < 158) {
			...
			buttons: [
				...
				{
					text: FR.T('Sign in'),  tabIndex: 4, cls: 'fr-btn-default fr-btn-primary', style: 'font-size:13px;margin-right:0',
					handler: function(){FR.submitForm();}
				},
				{
					text: FR.T('SSO'),  tabIndex: 5, cls: 'fr-btn-default', style: 'font-size:13px;margin-left:5px',
		...
		if (Settings.message) {
			new Ext.ux.prompt({text: Settings.message, callback: function() {FR.win.frm.usrField.focus(true);}});
		}
		if (Settings.ssoOnly) {
...
Ext.onReady(function() {
	...
	if (Ext.isAndroid && Settings.androidAppURL &&
